<div class="relative w-full flex box-border h-[44px]">
    <label title="Click to upload" for="{{ $uniqueId }}" class="cursor-pointer flex items-center gap-4 px-6 before:border-gray-400/60 hover:before:border-gray-300 group before:bg-gray-100 before:absolute before:inset-0 before:rounded-md before:border before:border-dashed before:transition-transform before:duration-300 hover:before:scale-100 active:duration-75 active:before:scale-95">
        ...
        <div class="relative">
            <span class="block text-[14px] pt-2 font-semibold relative text-blue-900 group-hover:text-blue-500">
                {{ $title ?? 'Upload sound' }}
            </span>
            <span id="message_{{ $uniqueId }}" class="mt-0.5 block text-sm text-blue-900 h-[20px]"></span>
        </div>
    </label>
