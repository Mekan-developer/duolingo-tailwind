        ...
        class="w-full mx-auto bg-[var(--bg-color-non-active)] p-6 rounded-md" onsubmit="disableButton()">
            ...
            <div class="px-4 py-6 mt-10 bg-white rounded-sm">
                ...
                <div class="my-2">
                    <div class="grid grid-cols-2 gap-4">
                        @foreach ($locales as $locale)
                            <div class="flex flex-col w-full">
                                <label for="'word'.{{$locale->id}}">Word {{$locale->name}}</label>
                                <textarea id="'word'.{{$locale->id}}" name="text[{{$locale->locale}}]" class="font-normal rounded-md bg-gray-50" cols="30" rows="4" placeholder="Text {{$locale->name}}">{{ $grammar->translate("text",$locale->locale) }}</textarea>
                    ...
                    <div class="grid grid-cols-2 gap-4 my-4" wire:ignore>
                        @foreach ($locales as $locale)
                        <div class="flex flex-col w-full">
                            <label class="font-normal" >Hint for {{$locale->name}}</label>
                            <textarea name="hint[{{$locale->locale}}]" class="letter font-normal rounded-md bg-gray-50" cols="30" rows="4" placeholder="Hint {{$locale->name}}">{{ $grammar->translate("hint",$locale->locale) }}</textarea>
                    ...
                    @foreach ($text_correct_parts as $key => $value)
                        <div class="grid grid-cols-2 gap-4 my-2 " wire:ignore>
                            <div class="flex flex-col w-full mb-2">
                                ...
                                @include('includes.exerciseParts.edit.english_text',['name'=>'text_correct_parts['.$loop->iteration.']','title' => 'text correct parts '.$loop->iteration ,'placeholder' => 'Text correct parts','value' => $value])
                            ...
                            <div class="flex flex-row items-end mb-2 ">
                                <div class="flex flex-col flex-1">
                                    @include('includes.exerciseParts.edit.english_text',['name'=>'text_incorrect_parts['.$loop->iteration.']','title' => 'text incorrect parts '.$loop->iteration,'placeholder' => 'Text incorrect parts','value' => $text_incorrect_parts[$key]])
