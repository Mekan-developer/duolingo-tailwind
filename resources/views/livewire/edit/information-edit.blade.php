<div>
    <div class="p-6 bg-white">
        ...
        <form action="{{route('information.update',['information' => $information->id])}}" method="POST" onsubmit="disableButton()">
            @csrf
            @method("PATCH")
            <div class="container flex flex-col gap-10 p-4 mx-auto bg-gray-200">
                <div class="flex w-full gap-10">
                    <div class="flex-1">
                        ...
                        @include('includes.information-includes.edit-lesson-checkbox',['lessons' => $lessons])
                        @error('lesson_ids')
                    ...
                    <div class="flex-1">
                        ...
                        @include('includes.information-includes.edit-exercise-checkbox',['exercises' => $exercises])
                        @error('exercise_ids')
                    ...
                    <div class="grid grid-cols-2 gap-10" wire:ignore>
                        @foreach ($locales as $locale)
                            <div class="flex flex-col text-[var(--bg-color-active)] my-1 py-2">
                                ...
                                <textarea  class="letter h-[300px]" id="HTML[{{$locale->locale}}]" name="information[{{ $locale->locale }}]"  placeholder="Informations {{$locale->name}}">{{ $information->translate('information',$locale->locale)}}</textarea>
                            </div>
                        @endforeach
                    </div>
